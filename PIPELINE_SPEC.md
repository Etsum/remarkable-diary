...
2. **Blank write-on PNGs** — **one PNG per page type**, no dates, no category tabs, nav &
...
```
 Figma  ──export──▶  6 master SVGs (text NOT outlined, id attrs ON)
   ...
   │        hdr-nav, footer-right, hdr-meta text → render 1 PNG/type│
   └──────────────────────────────────────────────────────────────┘
```
...
| D6 | Blank rail / chrome | Keep month tabs (static, no active highlight); **remove** category tabs, `hdr-nav`, `footer-right`; **clear** the text inside `hdr-meta` (keep the box so it's writable) |
...
(`hdr-big`, `hdr-month-name`, `hdr-meta-top/-bottom`, nav, toggle, checks); frame paths
...
| Day | `hdr-nav-prev-bg` / `-next-bg` | prev/next `day-…` |
...
4. **Remove** `hdr-nav` (the prev/next arrows — non-functional as a background).
5. **Remove** `footer-right` (the "↳ …" link text).
6. **Clear** the text inside `hdr-meta` (`hdr-meta-top`/`-bottom`) but **keep** the datebox
...
7. Render to **PNG at 1404×1872** via `cairosvg.svg2png(... output_width=1404,
...
### 13.2 Post-build
- Click an INDEX chip, month tab, mini-cal date, week number, nav arrow, rail category tab
...
- Blank PNGs: no numbers, no category tabs, no nav arrows, no footer-right link, hdr-meta
